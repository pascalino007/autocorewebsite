...
function calcTotals(items: ICartItem[]): ICartTotal[] {
    ...
    const deliveryTotal = items.reduce((acc, item) => acc + (item.deliveryFee || 0) * item.quantity, 0);
...
function cartBaseReducer(state = initialState, action: CartAction): ICartState {
    switch (action.type) {
    case CART_ADD_ITEM:
        return addItem(state, action.product, action.options, action.quantity, action.deliveryFee);
